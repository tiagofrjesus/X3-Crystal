<#
.SYNOPSIS
  Converte TEB_PIECE.rpt de paisagem para retrato A4 e acrescenta a info da empresa no rodapé.
...
param(
  ...
  [string]$WorkDsn = "TEST_TEB211",            # DSN ODBC local alcançável p/ build (schema TEB)
...
$cs = Get-Content (Join-Path $PSScriptRoot 'X3RptPiecePortrait.cs') -Raw -Encoding UTF8
Add-Type -TypeDefinition $cs -ReferencedAssemblies $refs -Language CSharp

$dir = Split-Path -Parent $OutPath
if($dir -and -not (Test-Path $dir)){ New-Item -ItemType Directory -Force -Path $dir | Out-Null }

$res = [X3RptPiecePortrait]::Build($RptPath,$OutPath,$WorkDsn,$WorkDb,$WorkUser,$WorkPass)
Write-Host "LOG: $res"
